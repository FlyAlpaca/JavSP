...
def get_poster_size(image_shape: tuple[int, int], ratio: float) -> tuple[int, int]:
    ...
    (poster_w, poster_h) = (
        (int(fanart_h / ratio), fanart_h) if fanart_h / fanart_w < ratio else (fanart_w, int(fanart_w * ratio))
    )  # 图片太“瘦”时以宽度来定裁剪高度
