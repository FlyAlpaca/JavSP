...
arg_parser = ArgumentParser(prog="config migration", description="migration your javsp config to yaml")
...
try:
    ...
    with open(args.input, encoding="gbk") as f:
        cfg.read_file(f)
...
summarizer:
  ...
  censor_options_representation: ['{cfg["NamingRule"]["text_for_uncensored"]}', '{cfg["NamingRule"]["text_for_censored"]}', '{
